...
describe("Certificate Integration Tests", () => {
  ...
  describe("Certificate PDF Generation Integration", () => {
    it("should handle PDF generation errors gracefully", async () => {
      vi.mocked(pdfCertificateGenerator.generatePDFCertificate).mockRejectedValue(new Error("PDF generation failed"));
      ...
      await expect(pdfCertificateGenerator.generatePDFCertificate(mockCertificate)).rejects.toThrow("PDF generation failed");
    });
  });
  ...
  describe("Certificate API Error Handling", () => {
    ...
    it("should handle service errors gracefully", async () => {
      ...
      expect(generateData.error).toBe("Order not found");
      ...
      expect(getData.error).toBe("Database error");
    });
  ...
  describe("Certificate Data Validation", () => {
    ...
    it("should handle certificates with missing metadata", async () => {
      ...
      vi.mocked(certificateService.getCertificateById).mockResolvedValue(certificateWithoutMetadata);
      const request = mockRequest({}, "GET", { id: "test-cert-id" });
      const response = await GET(request);
      const certificate = await response.json();
  ...
  describe("Certificate Status Management", () => {
    it("should handle different certificate statuses", async () => {
      ...
      vi.mocked(certificateService.getCertificateById).mockResolvedValueOnce(activeCertificate).mockResolvedValueOnce(revokedCertificate);
