...
export default function BiomassOnlyPage() {
  ...
  const computeStats = (prediction: number[][], shape: [number, number], bounds: Bounds, mask?: number[][]) => {
    const [h, w] = shape;
    ...
    for (let i = 0; i < h; i++) {
      for (let j = 0; j < w; j++) {
        ...
        if (currentMask && maskI >= 0 && maskJ >= 0 && currentMask[maskI] && currentMask[maskI][maskJ] > 0) {
          forestPixels++;
          forestMg += pixelMg;
  ...
  // Initialize Leaflet Draw when map is ready
  ...
  const paint = (e: MouseEvent) => {
    if (!paintStateRef.current.isPainting || !editMode || !forestMask || !forestMaskShape || !layersRef.current.rectangle || !canvasRef.current || !mapRef.current) return;
    ...
    for (let i = Math.max(0, Math.floor(maskY - brushRadiusY)); i < Math.min(forestMaskShape[0], Math.ceil(maskY + brushRadiusY)); i++) {
        for (let j = Math.max(0, Math.floor(maskX - brushRadiusX)); j < Math.min(forestMaskShape[1], Math.ceil(maskX + brushRadiusX)); j++) {
            newMask[i][j] = value;
        }
    }
  ...
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    canvas.addEventListener('mousedown', startPainting as any);
    canvas.addEventListener('mousemove', paint as any);
    canvas.addEventListener('mouseup', stopPainting as any);
    canvas.addEventListener('mouseleave', stopPainting as any);
    return () => {
      canvas.removeEventListener('mousedown', startPainting as any);
      canvas.removeEventListener('mousemove', paint as any);
      canvas.removeEventListener('mouseup', stopPainting as any);
      canvas.removeEventListener('mouseleave', stopPainting as any);
    };
  ...
  const startAnalysis = async () => {
      if (!selectedBounds) { setError("Draw a rectangle first."); return; }
      setError(null); setLoading(true); setOverlayUrl(null);
  ...
  const undo = () => {
     if (editHistoryIndex > 0) {
         ...
         if (biomassPrediction && biomassShape && selectedBounds) computeStats(biomassPrediction, biomassShape, selectedBounds, editHistory[idx]);
     }
  };
  const redo = () => {
  ...
  const clearAll = () => {
      ...
      setForestMask(null);
      setStats({ totalAreaKm2: null, totalBiomassMg: null, meanBiomassDensity: null, forestAreaKm2: null, forestCoveragePct: null, forestBiomassMg: null });
             ...
             <div className="flex flex-col md:flex-row h-[calc(100vh-90px)] md:h-[calc(100vh-130px)] overflow-hidden rounded-none md:rounded-lg md:border md:border-[#1e3a2a]" style={{ background: "#0f1419" }}>
                 ...
                 <div className="w-full md:w-[800px] flex flex-col border-t md:border-t-0 md:border-l border-[#1e3a2a] bg-[#151b28]">
                      ...
                      <div className="flex-1 min-h-[260px] max-h-[600px] flex items-center justify-center overflow-hidden border-b border-[#1e3a2a] px-3">
                         {overlayUrl ? (
                             <img src={overlayUrl} className="max-w-full max-h-full rounded border border-[#1e3a2a]" alt="Biomass Overlay" />
                         ) : loading ? (
                             <div className="flex flex-col items-center gap-3">
                                 <div className="w-12 h-12 border-4 border-[rgba(16,185,129,0.2)] border-t-[#10b981] rounded-full animate-spin" />
                                 <div className="text-center">
                                     <div className="text-[13px] font-medium text-[#10b981]">Processing Biomass Analysis...</div>
                                     <div className="text-[11px] text-[#7a8fa3] mt-1">This may take several minutes for large areas</div>
                                 </div>
                             </div>
                         ) : (
                      ...
                      <div className="flex w-full min-h-[360px]">
                          <div className="w-[260px] border-r border-[#1e3a2a] bg-[rgba(15,23,42,0.98)] p-4 overflow-y-auto">
                               <div className="text-[13px] font-semibold text-[#10b981] mb-3 uppercase">📊 Analysis</div>
                               <Button 
                               ...
                               <div className="text-[13px] font-semibold text-[#10b981] mb-3 uppercase">🖌️ Editor</div>
                               <div className="grid grid-cols-2 gap-2 mb-2">
                                   <Button size="sm" variant={editMode === 'add' ? 'default' : 'outline'} onClick={() => setEditMode(editMode === 'add' ? null : 'add')} className={editMode === 'add' ? 'bg-[#10b981] text-black' : 'text-gray-400 border-gray-700'}>➕ Add</Button>
                                   <Button size="sm" variant={editMode === 'remove' ? 'default' : 'outline'} onClick={() => setEditMode(editMode === 'remove' ? null : 'remove')} className={editMode === 'remove' ? 'bg-[#10b981] text-black' : 'text-gray-400 border-gray-700'}>➖ Remove</Button>
                               </div>
                               <div className="grid grid-cols-2 gap-2">
                                   <Button size="sm" variant="outline" onClick={undo} disabled={editHistoryIndex <= 0} className="text-gray-400 border-gray-700">↶ Undo</Button>
                                   <Button size="sm" variant="outline" onClick={redo} disabled={editHistoryIndex >= editHistory.length - 1} className="text-gray-400 border-gray-700">↷ Redo</Button>
                               </div>
                               
                               <div className="mt-3">
                                   <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Brush Size</span><span>{brushSize}px</span></div>
                                   <input type="range" min="5" max="50" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-full accent-[#10b981]" />
                               </div>

                               <div className="my-4 h-px bg-[#1e3a2a]" />
                               
                               <Button variant="secondary" className="w-full mb-2 bg-[#1e3a2a] text-[#10b981] hover:bg-[#2d5a3d]" onClick={() => setIsSaveDialogOpen(true)} disabled={!forestMask}>
                                   💾 Save as Forest
                               </Button>
                               <Button variant="destructive" className="w-full bg-red-900/20 text-red-400 border border-red-900 hover:bg-red-900/40" onClick={clearAll}>Clear All</Button>
                          </div>
                          ...
                          <div className="flex-1 p-4 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-3 content-start">
                               ...
                               <div className="col-span-full mt-2 bg-green-900/10 border border-green-900/20 rounded p-3">
                                   ...
                                   <div className="text-2xl font-bold text-[#10b981]">{stats.forestBiomassMg ? (stats.forestBiomassMg / 3.67).toFixed(2) : '-'} tCO₂e</div>
                               </div>
                          </div>
                      </div>
                 </div>
...
function StatCard({ title, value, unit }: { title: string, value: number | null, unit: string }) {
    return (
        <div className="bg-green-900/5 border border-green-900/20 rounded p-2">
            ...
            <div className="text-xl font-bold text-[#10b981]">{value !== null ? `${value.toFixed(2)} ${unit}` : '-'}</div>
        </div>
    )
}
