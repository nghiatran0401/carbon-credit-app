    ...
    <div className="container mx-auto flex flex-col items-center justify-center min-h-[70vh] py-12 px-4">
      ...
      {!loading && !error && order && payment && (
        <div className="bg-white rounded shadow p-4 mb-4 w-full max-w-md">
          ...
          <div className="mb-2 text-sm text-gray-700">
            Total Paid: <span className="font-semibold">${payment.amount.toFixed(2)}</span>{' '}
            {payment.currency}
          </div>
          <div className="mb-2 text-sm text-gray-700">
            Paid At:{' '}
          ...
          <ul className="list-disc pl-6 text-gray-700">
            {order.items?.map((item: OrderItem) => (
              <li key={item.id}>
                {item.quantity} x Credit #{item.carbonCreditId} @ ${item.pricePerCredit.toFixed(2)}{' '}
                each (Subtotal: ${item.subtotal.toFixed(2)})
              </li>
            ))}
          </ul>
        </div>
            ...
            <div className="flex items-start space-x-3 mb-4 p-3 bg-purple-50 rounded-lg border border-purple-200">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-purple-700 mb-2">
                  Transaction permanently recorded to immutable database for tamper-proof audit
                  trail.
                </p>

                {auditLoading && (
                  <div className="mt-2 text-xs text-purple-600">Loading audit record...</div>
                )}
                ...
                {!auditLoading && audit && (
                  <div className="mt-3 space-y-2">
                    <div className="bg-white p-2 rounded border border-purple-200">
                      ...
                      <div className="font-mono text-[10px] break-all text-gray-800">
                        {audit.hash}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="bg-white p-2 rounded border border-purple-200">
                        ...
                        <div className="text-xs font-mono">
                          ${audit.transactionData.totalPrice.toFixed(2)}
                        </div>
                      </div>
                      <div className="bg-white p-2 rounded border border-purple-200">
                        <div className="text-[10px] text-purple-600 font-medium">Paid At</div>
                        <div className="text-xs font-mono">
                          {new Date(audit.transactionData.paidAt).toLocaleString()}
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex items-start space-x-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-700">
                  Transaction relationships mapped to graph database for complete carbon credit
                  provenance tracking.
                </p>
